...
export default function WrapsTitleSection({ initialWraps = [], initialNavbarData = [], onWrapChange = null }) {
    const [wraps, setWraps] = useState(initialWraps);
    const [navbarData, setNavbarData] = useState(initialNavbarData || []);
    const [loading, setLoading] = useState(initialWraps.length === 0);
    const [carousel, setCarousel] = useState(() => {
        if (initialWraps.length > 0) {
            const initialCards = initialWraps.map((_, i) => {
                let diff = i;
                if (diff > initialWraps.length / 2) diff -= initialWraps.length;
                if (diff < -initialWraps.length / 2) diff += initialWraps.length;
    ...
    useEffect(() => {
        if (initialWraps.length > 0) return;
        const fetchWraps = async () => {
            try {
                const data = await client.fetch(`*[_type == "wrapsPage"][0].wrapsCarousel[] {
          name,
          image,
          boost
        }`);
    ...
    useEffect(() => {
        if (onWrapChange) {
            onWrapChange(carousel.center);
        }
    }, [carousel.center, onWrapChange]);
            ...
            `}</style>
                ...
                >
                  ...
                  `,
                    ...
                    {/* Laptop-only: full multi-card stacked carousel */}
                    {carousel.cards.map((card) => {

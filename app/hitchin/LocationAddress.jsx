...
export default function LocationAddress({ location = "hitchin" }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            console.log(`[LocationAddress] Logic starting for location: "${location}"`);
            try {
                ...
                if (!allDocs || allDocs.length === 0) {
                    console.warn("[LocationAddress] No mapSection documents found in Sanity.");
                    setLoading(false);
                    return;
                }

                console.log("[LocationAddress] Documents in Sanity:", allDocs.map(d => d.slug?.current));

                const locLower = location.toLowerCase();

                // Advanced matching logic:
                // 1. Exact match (case insensitive)
                let match = allDocs.find(doc => doc.slug?.current?.toLowerCase() === locLower);

                // 2. Slug contains the location prop (e.g. "hitchin-peckers" matches "hitchin")
                if (!match) {
                    match = allDocs.find(doc => doc.slug?.current?.toLowerCase().includes(locLower));
                }
                ...
                if (match) {
                    console.log(`[LocationAddress] ✅ SUCCESS: Found "${match.name}" for "${location}"`);
                    setData(match);
                } else {
                    console.error(`[LocationAddress] ❌ ERROR: No match for "${location}"`);
                    setData(null);
                }
            } catch (error) {
    ...
    if (!data) {
        return (
            <div className="flex flex-col justify-center items-center min-h-[40vh] text-white">
                <p className="text-xl font-peakers mb-4">Location information not found for "{location}"</p>
                <p className="text-sm opacity-50">Please ensure a document of type "Map Section" exists in Sanity with a slug matching or starting with "{location}".</p>
            </div>
        );
    }
